...
const JobPage = () => {
  ...
  }, [currentPage]); // Recreate fetchJobs only if currentPage changes

  // Fetch jobs when currentPage changes
  useEffect(() => {
    fetchJobs();
  }, [currentPage, fetchJobs]); // Include fetchJobs in the dependencies
  ...
  return (
    <div className="flex flex-row md:flex-row h-screen bg-white">
      ...
      {isModalOpen && <AddJobModal onClose={() => setIsModalOpen(false)} onSubmit={() => fetchJobs()} />}
    </div>
  );
};
