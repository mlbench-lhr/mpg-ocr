...
export async function GET(req: NextRequest) {
    ...
    try {
        ...
        if (!fileId || !fileTable) {
            return NextResponse.json({ message: "Missing fileId or fileTable" }, { status: 400 });
        }
        ...
        const tableCheckResult = await connection.execute(tableCheckQuery, [fileTable.toUpperCase()]);

        if (!tableCheckResult.rows || tableCheckResult.rows.length === 0) {
            return NextResponse.json({ message: "Invalid fileTable name" }, { status: 400 });
        }

        const result = await connection.execute<FileRow>(
            `SELECT FILE_ID, FILE_DATA FROM ${fileTable} WHERE FILE_ID = :fileId`,
            { fileId },
        ...
        if (!row.FILE_DATA) {
            return NextResponse.json({ message: "File data is empty" }, { status: 404 });
        }
