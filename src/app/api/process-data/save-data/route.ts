...
export async function POST(req: Request) {
    try {
        ...
        // Extract valid job IDs (skip empty jobIds)
        const validJobIds = [...new Set(dataArray
            .map(data => data.jobId)
            .filter(jobId => jobId && jobId.trim() !== ""))] // Remove empty or null jobIds
            .map(id => new ObjectId(id));

        const jobs = await db.collection("jobs").find({ _id: { $in: validJobIds } }).toArray();
        const jobMap = jobs.reduce<Record<string, WithId<Document>>>((map, job) => {
            map[job._id.toString()] = job;
            return map;
        }, {});
        ...
        for (const data of dataArray) {
            ...
            // If jobId is empty, set jobName to empty string
            if (!jobId || jobId.trim() === "") {
                data.jobId = ""; // Ensure jobId is an empty string
                data.jobName = ""; // Assign empty string to jobName
            } else {
                // Fetch job name if jobId exists in DB
                const job = jobMap[jobId];
